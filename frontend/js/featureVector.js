...
const FeatureVector = (() => {
    ...
    // INITIALIZE FEATURE VECTOR - Called after component HTML loads
    async function initializeFeatureVector(){
        console.log("Feature Vector JS Connected");

        try{
            const password = localStorage.getItem("analyzedPassword") || "Password123";
            ...
            console.log("Feature Vector Response:", data);
            const features = data.features;

            if(!features){
                console.error("No feature vector data received");
                return;
            }
            ...
            console.error("Feature Vector Error:", error);
        }
    }
...
window.initializeFeatureVector = FeatureVector.initializeFeatureVector;