...
async function initializeRecommendation(){
    ...
    try{
        ...
        const previousPasswordStored =
        localStorage.getItem("analyzedPassword");

        const comparisonPasswordStored =
        localStorage.getItem("comparisonPassword");

        const analyzedPassword =
        comparisonPasswordStored
        ||
        previousPasswordStored
        ||
        "Password123";

        // Only compare when a comparison test actually happened.
        // A single Initial Test password has nothing to compare against.
        ...
        if(list){
            ...
            // =====================================
            // COMPARISON RESULT
            // Only appears when a comparison test was
            // actually run (backend returns non-null
            // password_comparison in that case).
            // =====================================

            if(
                data.password_comparison &&
                data.password_comparison.message
            ){
                ...
                comparisonItem.classList.add(
                    "comparison-message"
                );


                comparisonItem.dataset.status =
                data.password_comparison.status || "";


                comparisonItem.textContent =
                data.password_comparison.message;
